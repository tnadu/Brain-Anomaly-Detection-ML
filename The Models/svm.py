...
from sklearn.svm import LinearSVC       # the model in use
...
for c in [2 ** p for p in range(-4, 5)]:
    ...
    validationPredictions = svmClassifier.predict(validationImages)
    ...
    with open('svm-reports.txt', 'a') as reportFile:
        reportFile.write(reportedMetrics)

    print('Predicting test set initiated')
    predictions = svmClassifier.predict(testImages)
    ...
    predictions.to_csv(f'svm-c-{c}.csv', index=None)
